...
/**
  ...
  */
case class Settings(timeout: FiniteDuration = 1.second, timeoutFactor: Double = 1.0, charset: Charset = StandardCharsets.UTF_8) {
  require(timeoutFactor >= 1.0 && timeoutFactor < Long.MaxValue && !timeoutFactor.isInfinite && !timeoutFactor.isNaN,
    "Time factor must be >=1, < Long.MaxValue, and not Infinity or NaN.")
