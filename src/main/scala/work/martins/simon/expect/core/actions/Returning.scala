...
//   case class Returning[R](result: => R) extends AbstractReturning[R]
...
final case class ActionReturningAction[R, +T](parent: Returning[R], resultAction: R => AbstractReturning[T]) extends AbstractReturning[T] {
  ...
  def structurallyEquals[TT >: T, W[+X] <: When[X]](other: Action[TT, W]): Boolean = other.isInstanceOf[ActionReturningAction[?, ?]]
}